...
async def cleanup_user_cache():
    while True:
        async with _cache_lock:
            current_time = datetime.utcnow().timestamp()
            expired = [token for token, (_, exp) in _user_cache.items() if current_time > exp]
            for token in expired:
                _user_cache.pop(token, None)
            logger.debug(f"Cleaned {len(expired)} expired tokens from cache")
...
async def login(login_request: LoginRequest, session_key_b64: str) -> TokenResponse:
    try:
        if len(session_key_b64) < 24:  # Minimum for 16-byte key
            logger.warning("Invalid session key length")
            raise HTTPException(status_code=400, detail="Invalid session key format")
        session_key = base64.b64decode(session_key_b64)
        if len(session_key) not in (16, 24, 32):  # AES key lengths
            logger.warning("Session key not 16, 24, or 32 bytes")
            raise HTTPException(status_code=400, detail="Invalid session key size")
        try:
            username = decrypt_data(base64.b64decode(login_request.username), session_key).decode("utf-8")
        except Exception:
            logger.error("Failed to decrypt username")
            raise HTTPException(status_code=400, detail="Invalid encrypted username")
        try:
        ...
        logger.error(f"Session key processing failed: {str(e)}")
